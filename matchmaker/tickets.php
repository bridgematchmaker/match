...
switch ($view) 
	{	
		case new_tickets:
			...
			$result2 = mysql_query("SELECT * FROM tickets  WHERE matchmaker_login='".$_SESSION['login_match']."' AND status='0';");
			$num = mysql_num_rows($result2);
		break;
		
		case all:
			// Узнаем количество всех доступных записей 
			$result2 = mysql_query("SELECT * FROM tickets WHERE matchmaker_login='".$_SESSION['login_match']."';");
			$num = mysql_num_rows($result2);
		break;
...
switch ($view) 
	{
		case new_tickets:
			$zapros = "SELECT * FROM tickets  WHERE matchmaker_login='".$_SESSION['login_match']."' AND status='0' ORDER BY ticket_id DESC LIMIT $quantity OFFSET $list;";
		break;
		
		case all:
			$zapros = "SELECT * FROM tickets  WHERE matchmaker_login='".$_SESSION['login_match']."' ORDER BY ticket_id DESC LIMIT $quantity OFFSET $list;";
		break;
	}
?>
<html>
				...
				for ($i = 0; $i<$num_result; $i++) 
					{
						...
						echo ("
								<tr>
									<td>
									  <a href='./ticket.php?ticket_id=$ticket_id'><span style='font-size:22px; margin-left:10px;' class='glyphicon glyphicon-folder-open'></span></a>
									</td>
									<td>
                                        $ticket_id
									</td>
