...
if($user_id == "")
	{
		echo("<script>alert('Ошибка параметра. Открываем все заказы по сайту');</script><script>javascript:window.location='./orders.php?view=all'</script>");
	}
...
$result2 = mysql_query("SELECT orders.*,clients.matchmaker_login FROM orders LEFT JOIN clients ON orders.user_id = clients.user_id WHERE orders.user_id='".$user_id."';");
...
$zapros = "SELECT orders.*,clients.matchmaker_login FROM orders LEFT JOIN clients ON orders.user_id = clients.user_id WHERE orders.user_id='".$user_id."' ORDER BY user_id DESC LIMIT $quantity OFFSET $list;";
