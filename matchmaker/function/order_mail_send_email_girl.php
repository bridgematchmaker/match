...
$mail_client = htmlspecialchars($mail_client);
$mail_client = mysql_escape_string($mail_client);
...
while($row_order = mysql_fetch_array($requirest_order)) 
	{
		...
		if($foto_client <> "")
			{
				$includeted_foto = "<img width='200' src='http://".$_SERVER['SERVER_NAME']."/".$foto_client."' >";
			}
	}
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////Отправка письма//////////////////////////////////////////////////////////////////////
	...
	$email_name = "auto_email_7"; // Название шаблона письма
	$email_send_to = $girl_email; // Получатель письма
	...
	$email_content = str_replace("{mail_client}", $mail_client, $email_content);
	...
	$email_content = str_replace("{includeted_foto}", $includeted_foto, $email_content);
...
$sql = "UPDATE orders SET mail_client='".$mail_client."', status='1' WHERE order_id='$order_id'";
...
echo	("
			<script>alert('Письмо успешно отпрвленно девушке');</script><script>javascript:window.location='../order.php?order_id=".$order_id."'</script>
		");
///////////////////////////////////////////////////////
?>
